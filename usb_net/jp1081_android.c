...
{
	printk(".......qf_write_reg() reg=0x%02x, value=0x%02x", reg, value);
...
static void qf_write_async_helper(struct usbnet *dev, u8 reg, u8 value, u16 length, void *data)
{
	...
	if (!urb) {
		printk(".......Error allocating URB in qf_write_async_helper!");
		return;
	}
	...
	if (status < 0) {
		printk(".......Error submitting the control message: status=%d",
		       status);
		kfree(req);
		usb_free_urb(urb);
	}
...
{
	printk(".......qf_write_async() reg=0x%02x length=%d", reg, length);
...
{
	printk(".......qf_write_reg_async() reg=0x%02x value=0x%02x", reg, value);
...
static void jp1081_android_mdio_write(struct net_device *netdev, int phy_id, int loc, int val)
{
	...
	printk(".......jp1081_android_mdio_write() phy_id=0x%02x, loc=0x%02x, val=0x%04x",
	       phy_id, loc, val);
...
/////////////////////////////////////////
///////////////////
static const struct net_device_ops jp1081_android_netdev_ops = {
...
static int jp1081_android_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	dev->net->netdev_ops = &jp1081_android_netdev_ops;
...
static int jp1081_android_rx_fixup(struct usbnet *dev, struct sk_buff *skb)
{
	...
	while (skb->len > QF_RX_OVERHEAD) {
		...
		skb_pull(skb, len + QF_RX_OVERHEAD);
	};
