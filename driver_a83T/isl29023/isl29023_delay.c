/*************************************************************************
	> File Name: isl29023.c
	> Author: shuixianbing
	> Mail: shui6666@126.com 
	> Created Time: 2017年03月06日 星期一 15时57分29秒
 ************************************************************************/
...
#define GPIO_INT GPIOG(13) 

#define ISL29023_COMMAND1 0x00
#define ISL29023_MODE_SHIFT (5)
#define ISL29023_MODE_MASK (BIT(7)|BIT(6)|BIT(5))
#define ISL29023_INT_FLAG_SHIFT (2)
#define ISL29023_INT_FLAG_MASK (BIT(2))
#define ISL29023_INT_PERSISTS_SHIFT (0)
#define ISL29023_INT_PERSISTS_MASK (BIT(0)|BIT(1))

#define ISL29023_COMMAND2 0x01
#define ISL29023_RES_SHIFT (2)
#define ISL29023_RES_MASK (BIT(3)|BIT(2))
#define ISL29023_RANGE_SHIFT (0)
#define ISL29023_RANGE_MASK (BIT(0)|BIT(1))
...
static const unsigned short normal_i2c[2] = {0x44, I2C_CLIENT_END};
...
static int __isl29023_read_reg(struct i2c_client *client, u32 reg, u8 mask, u8 shift){
	struct isl29023_data *data = i2c_get_clientdata(client);
	return (data->reg_cache[reg]&mask) >> shift;
}

static int __isl29023_write_reg(struct i2c_client *client, u32 reg, u8 mask,
			u8 shift, u8 val){
	struct isl29023_data *data = i2c_get_clientdata(client);
	int ret = 0;
	u8 tmp;
...
static int isl29023_get_int_persists(struct i2c_client *client){
	return __isl29023_read_reg(client, ISL29023_COMMAND1, ISL29023_INT_PERSISTS_MASK, ISL29023_INT_PERSISTS_SHIFT);
}

static int isl29023_set_int_persists(struct i2c_client *client, int int_persists){
	return __isl29023_write_reg(client, ISL29023_COMMAND1,ISL29023_INT_PERSISTS_MASK,ISL29023_INT_PERSISTS_SHIFT,int_persists);
}

//interrupt flag
static int isl29023_get_int_flag(struct i2c_client *client){
	return __isl29023_read_reg(client, ISL29023_COMMAND1,ISL29023_INT_FLAG_MASK,ISL29023_INT_FLAG_SHIFT);
}

static int isl29023_set_int_flag(struct i2c_client *client, int flag){
	printk("bshui isl29023_set int flag %d\n", flag);
	return __isl29023_write_reg(client, ISL29023_COMMAND1,ISL29023_INT_FLAG_MASK,ISL29023_INT_FLAG_SHIFT, flag);
}
...
static int isl29023_set_int_lt(struct i2c_client *client, int lt){
	...
	mutex_lock(&data->lock);
	ret = i2c_smbus_write_byte_data(client, ISL29023_REG_IRQ_TH_LO_LSB, lt&0xff);
...
static int isl29023_get_range(struct i2c_client *client){
	return __isl29023_read_reg(client, ISL29023_COMMAND2, ISL29023_RANGE_MASK,
				ISL29023_RANGE_SHIFT);
}
...
static int isl29023_set_mode(struct i2c_client *client, int mode){
	printk("bshui isl29023_set_mode %d\n", mode);
	return __isl29023_write_reg(client, ISL29023_COMMAND1, ISL29023_MODE_MASK,
				ISL29023_MODE_SHIFT, mode);
}

//power state
static int isl29023_set_power_state(struct i2c_client *client, int state){
	return __isl29023_write_reg(client, ISL29023_COMMAND1, ISL29023_MODE_MASK,
				ISL29023_MODE_SHIFT, state ? ISL29023_ALS_CONT_MODE : ISL29023_PD_MODE);
}
...
static int isl29023_get_adc_value(struct i2c_client *client){
	...
	return (((msb << 8) | lsb) * ((gain_range[range]*499)/data->rext)) >> bitdepth;
...
//interrupt persists
static ssize_t isl29023_show_int_persists(struct device *dev, struct device_attribute *attr, char *buf){
...
static ssize_t isl29023_store_int_persists(struct device *dev, struct device_attribute *attr, const char *buf, size_t count){
	struct i2c_client *client = to_i2c_client(dev);
	unsigned long val;
	int ret;

	if((strict_strtoul(buf, 10, &val) < 0) || (val >ISL29023_INT_PERSISTS_16))
	  return -EINVAL;
...
static DEVICE_ATTR(int_persists, S_IWUSR | S_IRUGO, isl29023_show_int_persists,
			isl29023_store_int_persists);

// interrupt flag
static ssize_t isl29023_show_int_flag(struct device *dev, struct device_attribute *attr, char *buf){
	struct i2c_client *client = to_i2c_client(dev);

	return sprintf(buf, "%i\n", isl29023_get_int_flag(client));
}

static ssize_t isl29023_store_int_flag(struct device *dev, struct device_attribute *attr, const char *buf, size_t count){
	struct i2c_client *client = to_i2c_client(dev);
	unsigned long val;
	int ret;
...
static DEVICE_ATTR(int_flag, S_IWUSR | S_IRUGO, isl29023_show_int_flag, isl29023_store_int_flag);

//interrupt lt
static ssize_t isl29023_show_int_lt(struct device *dev, struct device_attribute *attr, char *buf){
	struct i2c_client *client = to_i2c_client(dev);

	return sprintf(buf, "%i\n", isl29023_get_int_lt(client));
}

static ssize_t isl29023_store_int_lt(struct device *dev, struct device_attribute *attr, const char *buf, size_t count){
	struct i2c_client *client = to_i2c_client(dev);
	unsigned long val;
	int ret;
...
static DEVICE_ATTR(int_lt, S_IWUSR | S_IRUGO, isl29023_show_int_lt, isl29023_store_int_lt);

//interrupt ht
static ssize_t isl29023_show_int_ht(struct device *dev, struct device_attribute *attr, char *buf){
	struct i2c_client *client = to_i2c_client(dev);
	return sprintf(buf, "%i\n", isl29023_get_int_ht(client));
}

static ssize_t isl29023_store_int_ht(struct device *dev, struct device_attribute *attr, const char *buf, size_t count){
	struct i2c_client *client = to_i2c_client(dev);
	unsigned long val;
	int ret;
...
static DEVICE_ATTR(int_ht, S_IWUSR | S_IRUGO, isl29023_show_int_ht, isl29023_store_int_ht);

//range
static ssize_t isl29023_show_range(struct device *dev, struct device_attribute *attr, char *buf){
	struct i2c_client *client = to_i2c_client(dev);
	return sprintf(buf, "%i\n", isl29023_get_range(client));
}

static ssize_t isl29023_store_range(struct device *dev, struct device_attribute *attr, const char *buf, size_t count){
	struct i2c_client *client = to_i2c_client(dev);
	unsigned long val;
	int ret;
...
static DEVICE_ATTR(range, S_IWUSR | S_IRUGO, isl29023_show_range, isl29023_store_range);

//resolution
static ssize_t isl29023_show_resolution(struct device *dev, struct device_attribute *attr, char *buf){
	struct i2c_client *client = to_i2c_client(dev);
	return sprintf(buf, "%d\n", isl29023_get_resolution(client));
}

static ssize_t isl29023_store_resolution(struct device *dev, struct device_attribute *attr, const char *buf, size_t count){
	struct i2c_client *client = to_i2c_client(dev);
	unsigned long val;
	int ret;
...
static DEVICE_ATTR(resolution, S_IWUSR | S_IRUGO, isl29023_show_resolution, isl29023_store_resolution);

//mode 
static ssize_t isl29023_show_mode(struct device *dev, struct device_attribute *attr, char *buf){
	struct i2c_client *client = to_i2c_client(dev);
	return sprintf(buf, "%d\n", isl29023_get_mode(client));
}

static ssize_t isl29023_store_mode(struct device *dev, struct device_attribute *attr, const char *buf, size_t count){
	struct i2c_client *client = to_i2c_client(dev);
	unsigned long val;
	int ret;
...
static DEVICE_ATTR(mode, S_IWUSR | S_IRUGO, isl29023_show_mode, isl29023_store_mode);

//power state
static ssize_t isl29023_show_power_state(struct device *dev, struct device_attribute *attr, char *buf){
	struct i2c_client *client = to_i2c_client(dev);

	return sprintf(buf, "%d\n", isl29023_get_power_state(client));
}

static ssize_t isl29023_store_power_state(struct device *dev, struct device_attribute *attr, const char *buf, size_t count){
	struct i2c_client *client = to_i2c_client(dev);
	unsigned long val;
	int ret;
...
static DEVICE_ATTR(power_state, S_IWUSR | S_IRUGO, isl29023_show_power_state, isl29023_store_power_state);

//lux
static ssize_t isl29023_show_lux(struct device *dev, struct device_attribute *attr, char *buf){
	struct i2c_client *client = to_i2c_client(dev);
...
static ssize_t isl29023_show_int_lt_lux(struct device *dev, struct device_attribute *attr, char *buf){
	struct i2c_client *client = to_i2c_client(dev);
	//No LUX data if not operational
...
static ssize_t isl29023_store_int_lt_lux(struct device *dev, struct device_attribute *attr, const char *buf, size_t count){
	struct i2c_client *client = to_i2c_client(dev);
	struct isl29023_data *data = i2c_get_clientdata(client);
	...
	range = isl29023_get_range(client);
	bitdepth = (4 - isl29023_get_resolution(client)) * 4;
	lux_data = ((unsigned long)(val << bitdepth)) /((gain_range[range]*499)/data->rext);
	lux_data &= 0xffff;
	...
	mutex_lock(&data->lock);
	ret = i2c_smbus_write_byte_data(client, ISL29023_REG_IRQ_TH_LO_LSB, lsb);
	...
	ret = i2c_smbus_write_byte_data(client, ISL29023_REG_IRQ_TH_LO_MSB, msb);
	if(ret < 0){
		mutex_unlock(&data->lock);
		return ret;
	}
...
static DEVICE_ATTR(int_lt_lux, S_IWUSR | S_IRUGO, isl29023_show_int_lt_lux, isl29023_store_int_lt_lux);
//lux interrupt high threshold

static ssize_t isl29023_show_int_ht_lux(struct device *dev, struct device_attribute *attr, char *buf){
	struct i2c_client *client = to_i2c_client(dev);
	//No LUX data if not operational
	if(isl29023_get_mode(client) != ISL29023_ALS_ONCE_MODE && 
	   isl29023_get_mode(client) != ISL29023_ALS_CONT_MODE)
...
static ssize_t isl29023_store_int_ht_lux(struct device *dev, struct device_attribute *attr, const char *buf, size_t count){
	struct i2c_client *client = to_i2c_client(dev);
	struct isl29023_data *data = i2c_get_clientdata(client);
	unsigned long val, lux_data;
	int range, bitdepth, ret;
	...
	mutex_lock(&data->lock);
	ret = i2c_smbus_write_byte_data(client, ISL29023_REG_IRQ_TH_HI_LSB, lsb);
	...
	ret = i2c_smbus_write_byte_data(client, ISL29023_REG_IRQ_TH_HI_MSB, msb);
	if(ret < 0){
		mutex_unlock(&data->lock);
		return ret;
	}
...
static DEVICE_ATTR(int_ht_lux, S_IWUSR | S_IRUGO, isl29023_show_int_ht_lux, isl29023_store_int_ht_lux);
...
static int isl29023_init_client(struct i2c_client *client){
	...
	isl29023_set_int_flag(client, 1);
	isl29023_set_power_state(client, 1);

	return 0;
}

static void isl29023_work(struct work_struct *work){
	struct isl29023_data *data = container_of(work, struct isl29023_data, work);
...
static irqreturn_t isl29023_irq_handler(int irq, void *handle){
	...
	printk("bshui isl29023_irq_handler-----\n");

	return IRQ_HANDLED;
}

static int __devinit isl29023_probe(struct i2c_client *client, const struct i2c_device_id *id){
	...
	data->client = client;
	data->rext = 499; //499k Rext registor
	snprintf(data->phys, sizeof(data->phys), "%s", dev_name(&client->dev));
	...
	input_dev->name = "isl29023 light snesor";
	input_dev->id.bustype = BUS_I2C;
	input_dev->phys = data->phys;

	__set_bit(EV_ABS, input_dev->evbit);
	input_set_abs_params(input_dev, ABS_MISC, 0, gain_range[DEF_RANGE]*499/data->rext, 0, 0);
	...
	gpio_request_one(GPIO_INT, GPIOF_IN, "isl2903");
	//set irq type to edge falling 
	irq_set_irq_type(gpio_to_irq(GPIO_INT), IRQF_TRIGGER_FALLING);

	err = request_irq(gpio_to_irq(GPIO_INT), isl29023_irq_handler, 0, client->dev.driver->name, data);
	if(err < 0){
		printk("bshui failed to register irq \n");
		goto exit_free_input;
	}
...
/*	if(data->workqueue == NULL){
		printk("bshui couldn't create workqueue\n'");
		err = -ENOMEM;
		goto exit_free_interrupt;
	}*/
	schedule_delayed_work(&data->work, 1000);
	printk("bshui isl29023 probe----\n");

	return 0;
exit_free_interrupt:
	free_irq(gpio_to_irq(GPIO_INT), data);
exit_free_input:
...
static int isl29023_detect(struct i2c_client *client, struct i2c_board_info *info){
	...
	if(twi_id == adapter->nr){
		strlcpy(info->type, ISL29023_NAME, I2C_NAME_SIZE);
		return 0;
	}else
	  return -ENODEV;
}
