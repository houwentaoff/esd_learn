...
void GPS_original_signal(int fd)
{
	...
	while (1)
	{
		bzero(rcv_buf,sizeof(rcv_buf));
		{
			...
			{
#if 0
				if(!memcmp(rcv_buf,"$BDFKI", 6)){
					printf("%s",rcv_buf);
				}else
				if(!memcmp(rcv_buf,"$BDTXR", 6)){
					printf("%s",rcv_buf);
				}
				else if(!memcmp(rcv_buf,"$BDHZR", 6)){
					printf("%s",rcv_buf);
				}
				else if(!memcmp(rcv_buf,"$BDWAA", 6)){
					printf("%s",rcv_buf);
				}
				else if(!memcmp(rcv_buf,"$BDICI", 6)){
					printf("%s",rcv_buf);
				}
				else if(!memcmp(rcv_buf,"$BDICZ", 6)){
					printf("%s",rcv_buf);
				}
				else if(!memcmp(rcv_buf,"$BDGXM", 6)){
					printf("%s",rcv_buf);
				}
				else if(!memcmp(rcv_buf,"$BDBSI", 6)){
					printf("%s",rcv_buf);
				}
				else if(!memcmp(rcv_buf,"$BDZTI", 6)){
					printf("%s",rcv_buf);
				}
			#endif
				printf("%s", rcv_buf);
...
int main(int argc, char *argv[])
{
	...
	char BD_TXA[] = "$CCTXA,244882,1,0,你好,测试测试测试*52\r\n";
	char BD_CXA[] = "$BDCXA,1,01,0244883*71\r\n";
	char CCICA[] = "$CCICA,0,0*4b\r\n";
	int ret;
	int i;
	...
	ret = write(fd, &CCICA, sizeof(CCICA) );
	printf("ret=%d\n", ret);	
