...
/***
 *开启多标签循环识别
 * flagAnti:1 防碰识别,0 单标签识别
 *
 */
int UhfReaderStartInventory()
{
	int ret = 0;
	uint8_t flagAnti = 1;
	uint8_t initQ = 10;

	ret = UhfStartInventory(uhf_fd,flagAnti, initQ, flagCrc);
	return ret;
}
/**
 *停止RLM的当前操作
 */
int UhfReaderStopOperation(){
	int ret = 0;

	ret = UhfStopOperation(uhf_fd, flagCrc);
...
int main(void)
{
	...
	for(j=0; j<10; j++){
		...
		printf("读取标签失败\n");
		ret = UhfReaderReadInventory(&uLenUii, uUii);
